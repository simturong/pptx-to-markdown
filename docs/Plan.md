...
- [x] 크롭 정확도 검증 — 실제 PPTX 변환 후 검토 완료

### Phase 3 — 규칙 학습 (Few-shot 개선) ✅
- [x] 회사 PPTX 패턴 분석 → `rules.json` 구축 (v1.0 초안)
- [x] 복합 이미지 유형별 변환 예시 → `examples/` 폴더 구성 (3가지 유형)
- [x] 반복 등장 레이아웃 자동 인식 — 추가 PPTX 샘플 및 크롭 구조화 완료

### Phase 4 — 품질 평가 체계 ✅
- [x] 슬라이드별 변환 체크리스트 자동 생성 — evaluate.py 구현 완료
- [x] 누락 요소 감지 (이미지 수, 텍스트 블록 수 비교) — evaluate.py 검증 완료
...
## v3 계획: 통합 실행기 (run.py) 구축

사용자가 변환 요구사항에 따라 변환 속도와 문서 정밀도를 구분하여 실행할 수 있도록 통합 진입점(`run.py`)을 제공합니다.

### 모드별 세부 사항
1. **신속 모드 (Lite Mode / Express)**: V1 기반 작동. 파싱 텍스트 및 원본 이미지만 빠르게 추출하여 마크다운을 연동합니다.
2. **정밀 모드 (Pro Mode / Depth)**: V2 기반 작동. 복합 이미지의 부분 크롭 분할과 텍스트 표 구조화, 그리고 최종 산출물의 자동 품질 평가(`evaluate.py`) 및 오류 리포트 작성을 원스톱으로 처리합니다.

### 실행 방법
*   **신속 모드**: `python src/run.py input/sample.pptx output --mode lite`
*   **정밀 모드**: `python src/run.py input/sample.pptx output --mode pro`
