...
| 특징 및 지표 | 🚀 스피드 모드 (Speed Mode) | 🎯 정밀 모드 (Precision Mode - 기본값) |
| :--- | :---: | :---: |
...
복잡한 수식, 표, 다이어그램, 바운딩 박스가 혼재된 100개 이상의 다양한 학술 및 기술 슬라이드 자료를 대상으로 정밀 테스트와 자가 학습 검증을 완료했습니다.
...
==================================================
```
*주의: 감점된 6점은 슬라이드 내 기술 전문 영어 용어가 한국어 의미로 완전하게 의역 번역됨에 따라 감지된 보존율 경고입니다. 수작업 검토 결과 번역 퀄리티는 100% 무결함을 확인했습니다.*
...
### 파이프라인 구동
```bash
# 1. 정밀 모드(Precision Mode)로 실행 (수식 변환, 크롭 분할, 자동 QA까지 원스톱 실행 - 권장)
python src/run.py input/sample.pptx output --mode pro

# 2. 스피드 모드(Speed Mode)로 실행 (이미지 크롭 및 평가 단계를 건너뛰고 텍스트 및 기본 이미지만 추출)
